...
(define value-of
  (lambda (exp nameless-env)
    (cases expression exp
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;; PROBLEM 4 ;;;;;;;;;;;;;;;;;;;
      (cond-exp (controls exps)
                (if (or (null? controls) (null? exps))
                    (eopl:error "no values could be returned from cond") ; report error if no control is true
                    (let ((val1 (value-of (car controls) nameless-env))) ; val1 will becomes the value of first control
                          (if (expval->bool val1)
                              (value-of (car exps) nameless-env) ; it condition is true, evaluate the expression
                              (value-of (cond-exp (cdr controls) (cdr exps)) nameless-env) ; otherwise, recurse for the rest of them
                              ))))      
      ;;;;;;;;;;;;;;;;;;;;;;;; PROBLEM 4 ;;;;;;;;;;;;;;;;;;;
